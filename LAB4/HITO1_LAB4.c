...
void start_TIM()
{
	//-----------------------------Configuración del BT del TIM1----------------------------//

	go_TIM->CNT=0;
	go_TIM->PSC=8399; //Configuración Tu=0,1ms
	go_TIM->ARR=9; // la cuenta del TIMER llega hasta 1ms

	//-----------------------------configuración interna TIM1----------------------------------//

	go_TIM->CR1=0; //RESET
	go_TIM->CR2=0; //Deshabilitado no lo vamos a utilizar
	go_TIM->DIER=0; //RESET
	go_TIM->SMCR=0;// Deshabilitado no se va a utilizar
	go_TIM->CR1 |=(1<<2);//Activamos solo interrupción por update
	go_TIM->DIER |=(1<<0); // Activamos interrupción por update

	//------------------------------PUESTA EN MARCHA DEL TIM1-----------------------------------//

	go_TIM->EGR |=(1<<0);//Generamos un evento update
	go_TIM->SR=0; //Reset del flag
		//ACTIVAMOS el timer del deboucing mediante la interrupción generada por el botón PC13 //deboucing_TIM->CR1 |=(1<<0);
	//NVIC_SetPriority(TIM3_IRQn, 0);
	  NVIC_EnableIRQ(TIM1_UP_TIM10_IRQn); //NECESITO que el NVIC sepa en todo momento que está ocurriendo la interrupción
}
void clock_config()
{
...
void TIM5_IRQHandler()
{
  ...
  if(aux==1)
  {
	  go_TIM->CR1 |=(1<<0);//Timer de control de medición
	  ...
	  if(c==1)
	  {
		...
		// aux=0;//reinicio
		 ...
		 if(g==1)
		 {
		   ...
		   if(ADC1->SR & (1<<1)) //mientras no sea 1 el EOC (no haya llegado a FIN de conversión) el programa no realizará ningun printf
		   {
