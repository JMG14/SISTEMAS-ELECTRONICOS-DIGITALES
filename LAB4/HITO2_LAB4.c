...
void pwm_TIM()
{
	//---------------------------------------CONFIGURACIÓN BT TIMER5----------------------------------//
		...
		PWM_TIM->ARR=200-1; //20ms periodo
...
{
  /* USER CODE BEGIN 1 */
	char inicio[]="Vamos a controlar un SERVO mediante un POTENCIOMETRO\r\n";
		char res_string[50]="";
  /* USER CODE END 1 */
  ...
  while (1)
  {
	  if(p==1)
		         ...
		         PWM_TIM->CCR1=valor_DR*26/63; //No es necesario tanta resolución con 6 bit es suficiente ya que el POTENCIOMETRO no es tan preciso
	  	  		 p=0; //reset variable
	  	  	   }
	  	  sprintf(res_string,"el valor de ADC es: %d\r\n",valor_DR);
