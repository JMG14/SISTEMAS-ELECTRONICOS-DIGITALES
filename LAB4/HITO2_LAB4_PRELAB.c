...
//---------------------------------------MODO DE TRABAJO DEL ADC-------------------------------------------------------------//
   ...
   ADC1->CR2 |=(1<<1); // Queremos que esté haciendo conversiones continuamente (que se autodispare)
...
float f_res=0; //almacena valor de escalado
 ...
 // sprintf(res_string1, "Medida: %.1fV");
  ...
  while (1)
  {
       ...
       if(ADC1->SR & (1<<1))
		   { //mientras no sea 1 el EOC (no haya llegado a FIN de conversión) el programa no realizará ninguna acción
    	   pantalla=1;
