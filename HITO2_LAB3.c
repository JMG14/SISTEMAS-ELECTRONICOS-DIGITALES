...
char msg[]="\r\n Ejemplo con sensor HC SR-04 con IC\n\r";
...
/* USER CODE BEGIN PFP */
void servo_TIMER()
{ //-------------------------CONFIGURAR BT DEL TIMMER 4-----------------------------//
	/* Este TIMER simplemente va a mandar una señal PWM al servo para que se mueva como queremos
	 */
	servo_TIM->CNT=0; //la cuenta del timmer empieza en 0
	servo_TIM->PSC=8399; //conseguimos una resolución Tu=0,1ms
	servo_TIM->ARR=199; //conseguimos una demora de 20ms que es el tiempo suficiente para evitar los rebotes

  //----------------------------CONFIGURACIÓN INTERNA DEL TIMMER 4---------------------------//

	servo_TIM->CR1=0; //RESETEAMOS LAS CONFIGURACIONES
	servo_TIM->CR1 |=(1<<2)|(1<<7);//activamos modo ARPE y URS activado solo evento update genera DMA o interrupción "
	servo_TIM->SMCR =0; //DESACTIVADO
	servo_TIM->CR2=0; // DESACTIVADO

//---------CONFIGURACIÓN MODO TOC PARA GENERAR SEÑAL 20ms---------------------------//

	servo_TIM->CCMR2 =0; //Reset de TODAS las configuraciones
	servo_TIM->CCMR2 &=~(11<<0); //configuración <CANAL 3> como OUTPUT (TOC) vamos a sacar la señal PWM
	servo_TIM->CCMR2 |=(6<<4); //Configuramos como PWM con el primer semiciclo a "1"
	servo_TIM->CCER=0; //RESET de TODAS las posiciones
	//trig_TIM->CCER |=(1<<1); //activo a low
	servo_TIM->CCER |=(1<<8); //ACTIVAR el modo comparación del <CANAL 1>
	//servo_TIM->CCR3 = 4999;

//-----------------------------------INTERRUPCIONES-----------------------------//

	servo_TIM->EGR |=(1<<0); // Habilitamos el "update generation" para que reinicie el contador y genere una actualización
			// "update" de los registros ya que vamos a utilizar el flag y necesitamos que se actualice.
	servo_TIM->DIER=0;
	servo_TIM->DIER |=(1<<0); //Habilitamos la interrupción cuando ocurra evento UPDATE
	servo_TIM->SR &=~(1<<0);// limpiamos el flag del status registrer
...
//---------CONFIGURACIÓN MODO TOC PARA GENERAR SEÑAL 10us---------------------------//
	...
	trig_TIM->CCR1 = 35990; //Ponemos el CCR1 de manera que nos genere un DC=50% y esté encendido 0.5s
...
void clk_cnfig()
{
	...
	RCC->APB1ENR |=((1<<1)|(1<<0)|(1<<2));// reloj TIM3 y TIM2 y TIM4
}
void EXTII_CNFIG()
{
...
GPIOB->MODER &=~ (11 << (8*2)); //RESET PB8
GPIOB->MODER |= (10 << (8*2)); //salida AFR PB8
GPIOB->AFR[1] &=~(1111<<(0*4));//RESET DEL REGISTRO AF02 PB8 <CANAL 3>
GPIOB->AFR[1] |=(1<<(0*4)<<1);// configuramos columna AF02 del puerto B en pin 8 relacionado con TIM4 <CANAL 3>
...
void EXTI15_10_IRQHandler(void) {
  if (EXTI->PR & (1<<13)&&(aux==0))
  {
	   ...
	   servo_TIM->CR1 |=(1<<0); //ACTIVAMOS el TIMER 4 que es el controlador del SERVO
  ...
  while (1)
{
	 ...
	 HAL_Delay(1000);
	  ...
	  PUL=0; // Si pulso lo pongo a 0 para la próxima interrupción
	  Tiempo =  numTicks-480; // ese 480 es el flanco de subida, como el sensor es fijo el valor del flanco de subida es
	 // constante y con el flanco de bajada ya sabemos el tiempo en us
	  ...
	  c++;
	   HAL_Delay(1000);
	       switch (c)
	      	      	  {
	      	      	  case 1:
	      	      		 servo_TIM->CCR3=26; //posición final pulso 0º
	      	      	  HAL_Delay(1000);
	      	      	 break;
	      	      	  case 2:
	      	      	  servo_TIM->CCR3=21; //posición 45º
	      	      	 HAL_Delay(1000);
	      	       break;
	      	      	  case 3:
	      	      	  servo_TIM->CCR3=16;//posición 90º         26-4=22   22/2=11     11+4=16 ->90º
	      	      	HAL_Delay(1000);
	      	      break;
	      	      	  case 4:
	      	      	  servo_TIM->CCR3=9;//posición 135º
	      	      	 HAL_Delay(1000);
	      	       break;
	      	      	  case 5:
	      	      	 servo_TIM->CCR3=6; //posición inicial pulso 180º
	      	      	  HAL_Delay(1000);
	      	      	 break;
	      	      	  case 6:
	      	      	  servo_TIM->CCR3=10;//posición 135º
	      	      	 HAL_Delay(1000);
	      	      	 break;
	      	      	  case 7:
	      	      	  servo_TIM->CCR3=16;//posición 90º
	      	      	HAL_Delay(1000);
	      	      	  //c=0;
	      	      	  break;
	      	      	  }
