...
char presentacion[]="Vamos a realizar una conversion mediante TRGO obteniendo el valor PROMEDIO\r\n";
...
HAL_UART_Transmit(&huart2,(uint8_t *)presentacion,strlen(presentacion),HAL_MAX_DELAY);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  if(stream==1)
	  {
		  ...
		  for (j=0;j<4;++j)
		  {
			  valor_DR[j]=valor_DR[j]*3.3/4095; //necesitamos hacer escalado para saber el nivel de VOLTAJE que estamos cuantificando
			  suma= suma+valor_DR[j];

		  }
		promedio= suma/4;
		sprintf(res_string,"El promedio de las conversiones es: %.2f\r\n",promedio);
		HAL_UART_Transmit(&huart2,(uint8_t *)res_string,strlen(res_string),HAL_MAX_DELAY); //estamos imprimiendo por pantalla en el while por POLLING
        suma=0;
        promedio=0;
		stream=0;
	  }
